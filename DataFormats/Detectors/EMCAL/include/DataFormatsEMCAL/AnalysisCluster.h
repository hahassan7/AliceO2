...
class AnalysisCluster
{
 ...
 public:
  ...
  double getTrackDx(void) const { return mTrackDx; }
  double getTrackDz(void) const { return mTrackDz; }
  ...
  void setCellsAbsId(gsl::span<unsigned short> array);
  gsl::span<unsigned short> getCellsAbsId() { return mCellsAbsId; }
  ...
  void setCellsAmplitudeFraction(gsl::span<Double32_t> array);
  gsl::span<Double32_t> getCellsAmplitudeFraction() { return mCellsAmpFraction; }

  int getCellAbsId(int i) const
  {
    if (i >= 0 && i < mNCells)
      return mCellsAbsId[i];
    else
 ...
 protected:
  gsl::span<int> mLabels; ///< List of MC particles that generated the cluster, ordered in deposited energy.

  int mNCells = 0; ///< Number of cells in cluster.

  /// Array of cell absolute Id numbers.
  gsl::span<unsigned short> mCellsAbsId; //[mNCells]
  ...
  gsl::span<Double32_t> mCellsAmpFraction; //[mNCells][0.,1.,16]
